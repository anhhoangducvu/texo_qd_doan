...
def build(data):
    ...
    add_title(doc, f"Bổ sung cán bộ đoàn {data['loai_tu_van']}")
    add_tgd(doc)
    add_can_cu(doc, data)
    add_quyet_dinh_label(doc)

    p1 = build_dieu1_header(doc, "Bổ sung cán bộ", data["loai_tu_van"], data)
    fmt_body(p1.add_run(" gồm các cán bộ theo danh sách đính kèm."))   # ← Áp dụng fmt_body để đúng font

    add_dieu_2_3_4(doc, data["ten_tt"], data["loai_tu_van"])
    add_footer_table(doc)

    # Trang 2 – bảng danh sách (giống M01.2)
    doc.add_page_break()
...
def _add_danh_sach_page(doc, data):
    ...
    fb(p_ds.add_run("DANH SÁCH BỔ SUNG CÁN BỘ ĐOÀN TƯ VẤN"), bold=True)
